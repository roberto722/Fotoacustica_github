...
path_K = which(strcat(deviceId, '_bp_kernel.mat'));
if exist(path_K, 'file')
    load(which(strcat(deviceId, '_bp_kernel.mat')), 'K');
else
    fprintf('Kernel for backprojection reconstruction is being calculated...');
    probe = Probe(deviceId, false, false);
    K = bp_kernel((1:4100) ./ probe.DAC.frequency);
    save(strcat(deviceId, '_bp_kernel.mat'), 'K');
end
end
