function [rec_img, lCurveErrImg, lCurveErrReg] = reconstruct_model_based_residuals(model, sinogram, regularization, lambda_shearlet, lambda_tikhonov, lambda_laplacian, lambda_TV, lambda_L1_eye_reg, num_iterations_mb)

% Shearlet non-negative limited view reconstruction
if strcmp(regularization, 'L1_SHEARLET')
    lambda_shearlet
    [rec_img, lCurveErrImg, lCurveErrReg] = rec_nn_with_Shearlet_reg(model, sinogram, num_iterations_mb, lambda_shearlet);

% Non-negative reconstruction with L1 eye reg. matrix
elseif strcmp(regularization, 'L1_EYE_REG')
    [rec_img, lCurveErrImg, lCurveErrReg] = rec_nn_with_L1_eye_reg(model, sinogram, num_iterations_mb, lambda_L1_eye_reg);

% Non-negative reconstruction without regularization
elseif strcmp(regularization, 'NN_REC_WITHOUT_REG')
    [rec_img, lCurveErrImg, lCurveErrReg] = rec_nn_with_L2_reg(model, sinogram, num_iterations_mb, 0, [], [], 0, [], []);
...
elseif strcmp(regularization, 'L2_TIKHONOV_AND_LAPLACIAN')
    ...
    [rec_img, lCurveErrImg, lCurveErrReg] = rec_nn_with_L2_reg(model, sinogram, num_iterations_mb, lambda_tikhonov, RegL2, RegL2T, lambda_laplacian, RegL2_lap, RegL2T_lap);

% TV non-negative limited view reconstruction
elseif strcmp(regularization, 'TV_NN_REG')
    lambda_TV
    [rec_img, lCurveErrImg, lCurveErrReg] = rec_nn_with_TV_reg(model, sinogram, num_iterations_mb, lambda_TV);
